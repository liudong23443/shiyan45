...
# 创建自定义瀑布图函数来代替SHAP的瀑布图
def custom_waterfall_plot(shap_values, feature_names, max_display=10, figsize=(7, 5)):
    """
    绘制自定义瀑布图，确保中文显示正确
    """
    # 获取SHAP值
    ...
    # 只显示贡献最大的max_display个特征
    if max_display < len(indices):
    ...
    # 基础值
    ax.text(0, len(indices), f'f(x) = {base_value:.3f}', 
            ha='right', va='center', fontproperties=font_prop if font_path else None)
    
    # 逐个添加特征贡献
    for i, idx in enumerate(reversed(indices)):
        features_shown.append(feature_names[idx])
        values_shown.append(values[idx])
        
        # 文本颜色
        color = 'red' if values[idx] > 0 else 'blue'
        value_text = f'+{values[idx]:.2f}' if values[idx] > 0 else f'{values[idx]:.2f}'
        
        # 绘制条形
        if values[idx] > 0:
            ax.barh(i, values[idx], left=cumulative, height=0.5, color='red', alpha=0.7)
        else:
            ax.barh(i, values[idx], left=cumulative, height=0.5, color='blue', alpha=0.7)
        
        # 添加特征名称和值
        ax.text(-0.1, i, f'{feature_names[idx]} = {shap_values.data[0][idx]}', 
                ha='right', va='center', fontproperties=font_prop if font_path else None)
    ...
    # 最终预测值
    ax.text(cumulative, len(indices)-1, f'E[f(X)] = {cumulative:.2f}', 
           ha='left', va='center', fontproperties=font_prop if font_path else None)
    
    # 设置标题和轴标签
    ax.set_title('特征对预测的影响', fontproperties=font_prop if font_path else None)
    ax.set_xlabel('SHAP值', fontproperties=font_prop if font_path else None)
    ax.set_yticks([])
            ...
            try:
                ...
                try:
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        # 使用自定义瀑布图函数
                        fig = custom_waterfall_plot(shap_values, features_df.columns)
